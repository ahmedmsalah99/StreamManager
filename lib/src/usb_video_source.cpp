...
USBVideoSource::USBVideoSource(const VideoSourceConfig &config,
    ...
    : VideoSourceBase(config, node), camera_fps_(30.0), frame_size_(640, 480),
      camera_connected_(false) {
  std::cout << "Statistics reset in usb video" << std::endl;
}

USBVideoSource::~USBVideoSource() { shutdown(); }
...
double USBVideoSource::getFPS() const { return getAverageFPS(); }
...
cv::Size USBVideoSource::getFrameSize() const { return frame_size_; }
...
bool USBVideoSource::openCamera() {
  try {
    ...
    if (!capture_.isOpened()) {
      std::cerr << "Failed to open camera with device ID: " << config_.device_id
                << std::endl;
      return false;
    }
    ...
    std::cerr << "OpenCV exception while opening camera: " << e.what()
              << std::endl;
    return false;
  }
}
...
bool USBVideoSource::verifyCameraCapabilities() {
  ...
  if (test_frame.cols < 32 || test_frame.rows < 32) {
    std::cerr << "Camera verification failed: frame size too small"
              << std::endl;
    return false;
  }
