...
struct BufferConfig {
  size_t max_size =
      30; // with target_fps = 30 the buffer will hold 1 second of frames
  double target_fps = 30.0; // this will fill every 33 milliseconds
};

struct VideoSourceConfig {
  enum class SourceType { GAZEBO_ROS2, USB_CAMERA, MAVLINK };
...
private:
  ...
  stringToSourceType(const std::string &type_str) const;
