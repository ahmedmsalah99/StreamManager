...
int main(int argc, char* argv[]) {
    ...
    auto manager = std::make_shared<StreamManager>("config/default_config.yaml", "stream_manager_node");
    ...
    std::cout << "  Current frame: " << manager->getConfig().publishing.current_frame_topic << std::endl;
    std::cout << "  Delayed frame: " << manager->getConfig().publishing.delayed_frame_topic << std::endl;
    ...
    while (rclcpp::ok()) {
        ...
        if (frame_count % 60 == 0 && frame_count > 0) {
            ...
            if (latest_metadata) {
                std::cout << "  Latest frame metadata - Time: " << latest_metadata->timestamp_ns_system 
            ...
            if (metadata_by_idx) {
                std::cout << "  Oldest frame metadata - Time: " << metadata_by_idx->timestamp_ns_system << std::endl;
            }
            ...
            if (closest_metadata) {
                ...
                auto time_diff = static_cast<double>(now_ns - closest_metadata->timestamp_ns_system) / 1e9;
                std::cout << "  Closest metadata to 1s ago: Time " << closest_metadata->timestamp_ns_system 
                         << " (actual diff: " << time_diff << "s)" << std::endl;
                
                // Now get the actual frame from owned data using metadata
                auto frame_from_metadata = manager->getFrameFromOwnedData(*closest_metadata);
        ...
        if (frame_count % 120 == 0 && frame_count > 0) {
            ...
            if (buffered_frame) {
                std::cout << "  Successfully retrieved frame from buffer: " 
                         << buffered_frame->cols << "x" << buffered_frame->rows << std::endl;
            } else {
