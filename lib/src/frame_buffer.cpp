...
FrameBuffer::FrameBuffer(const BufferConfig &config)
    : max_buffer_size_(config.max_size), target_fps_(config.target_fps),
...
std::shared_ptr<cv::Mat> FrameBuffer::getClosestFrame(
...
std::shared_ptr<FrameMetadata> FrameBuffer::getClosestFrameMetadata(
    const std::chrono::steady_clock::time_point &target_time) const {
  std::lock_guard<std::mutex> lock(buffer_mutex_);
  ...
  auto closest_it = std::min_element(
      metadata_buffer_.begin(), metadata_buffer_.end(),
      [&target_time_ns](const std::shared_ptr<FrameMetadata> &a,
                        const std::shared_ptr<FrameMetadata> &b) {
        auto diff_a = std::abs(
            static_cast<int64_t>(a->timestamp_ns_system - target_time_ns));
        auto diff_b = std::abs(
            static_cast<int64_t>(b->timestamp_ns_system - target_time_ns));
...
FrameBuffer::getClosestFrameMetadataBySystemNs(uint64_t target_time_ns) const {
  std::lock_guard<std::mutex> lock(buffer_mutex_);
  if (metadata_buffer_.empty()) {
    return nullptr;
  }
  auto closest_it = std::min_element(
      ...
      [&target_time_ns](const std::shared_ptr<FrameMetadata> &a,
                        const std::shared_ptr<FrameMetadata> &b) {
        auto diff_a =
            std::llabs(static_cast<long long>(a->timestamp_ns_system) -
                       static_cast<long long>(target_time_ns));
        auto diff_b =
            std::llabs(static_cast<long long>(b->timestamp_ns_system) -
                       static_cast<long long>(target_time_ns));
        return diff_a < diff_b;
      });
  return *closest_it;
...
std::shared_ptr<FrameMetadata>
FrameBuffer::getFrameMetadataByIdx(size_t index) const {
  std::lock_guard<std::mutex> lock(buffer_mutex_);
...
FrameBuffer::getFrameFromOwnedData(const FrameMetadata &metadata) const {
  try {
    ...
    std::cerr << "Error reconstructing frame from shared memory: " << e.what()
              << std::endl;
    return nullptr;
  }
}
...
double FrameBuffer::getAverageFPS() const {
  ...
  auto time_span_ns = metadata_buffer_.back()->timestamp_ns_system -
                      metadata_buffer_.front()->timestamp_ns_system;
...
std::chrono::steady_clock::time_point FrameBuffer::getOldestFrameTime() const {
  ...
  auto time_since_epoch =
      std::chrono::nanoseconds(metadata_buffer_.front()->timestamp_ns_system);
...
std::chrono::steady_clock::time_point FrameBuffer::getNewestFrameTime() const {
  ...
  // Convert nanoseconds back to time_point
  auto time_since_epoch =
      std::chrono::nanoseconds(metadata_buffer_.back()->timestamp_ns_system);
  return std::chrono::steady_clock::time_point(time_since_epoch);
}
...
bool FrameBuffer::storeFrameInOwnedBuffer(const cv::Mat &frame,
                                          double current_fps) {
  try {
    ...
    auto metadata = std::make_shared<FrameMetadata>(
        ts_ns_system, current_fps, frame.cols, frame.rows, frame.channels(),
        frame.type(), data, frame_data_size);

    std::lock_guard<std::mutex> lock(buffer_mutex_);
    metadata_buffer_.push_back(metadata);
    return true;
  } catch (const std::exception &e) {
    std::cerr << "Error storing frame in owned buffer: " << e.what()
              << std::endl;
    return false;
  }
}
...
cv::Mat FrameBuffer::reconstructFrameFromOwnedData(
    const FrameMetadata &metadata) const {
  if (!metadata.owned_data || metadata.owned_data->empty()) {
    return cv::Mat();
  }
  return cv::Mat(metadata.height, metadata.width, metadata.cv_type,
                 (void *)metadata.owned_data->data())
      .clone();
}
