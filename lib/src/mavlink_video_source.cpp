...
MAVLinkVideoSource::MAVLinkVideoSource(const VideoSourceConfig &config,
    ...
    : VideoSourceBase(config, node), connection_string_(config.mavlink_address),
      streaming_(false), stream_requested_(false),
      current_stream_id_(DEFAULT_STREAM_ID), mavlink_connected_(false) {}

MAVLinkVideoSource::~MAVLinkVideoSource() { shutdown(); }
...
double MAVLinkVideoSource::getFPS() const { return getAverageFPS(); }
...
bool MAVLinkVideoSource::initialize() {
  ...
  if (!parseConnectionString()) {
    std::cerr << "Failed to parse MAVLink connection string: "
...
bool MAVLinkVideoSource::isConnected() const { return mavlink_connected_; }
...
std::string MAVLinkVideoSource::getSourceInfo() const {
  ...
  for (const auto &stream : available_streams_) {
    info << "  Stream " << stream.stream_id << ": " << stream.name << " ("
         << stream.width << "x" << stream.height << " @ " << stream.framerate
         << " FPS)\n";
  }
...
bool MAVLinkVideoSource::requestVideoStream() {
  ...
  // In a real implementation, you would send MAVLink VIDEO_START_STREAMING
  // message
...
//         result.push_back({s.stream_id, s.name, s.uri, s.width, s.height,
//         s.framerate, s.active});
//     }
//     return result;
// }
...
bool MAVLinkVideoSource::initializeMAVLink() {
  try {
    ...
    // Initialize available streams with default stream
    {
      ...
      available_streams_.push_back({DEFAULT_STREAM_ID, "Default Stream",
                                    connection_string_, 640, 480, 30.0, false});
    }
...
bool MAVLinkVideoSource::processVideoFrame(
...
void MAVLinkVideoSource::handleVideoStreamStatus(
    const std::vector<uint8_t> &payload) {
  // Placeholder parsing
}

cv::Mat
MAVLinkVideoSource::decodeVideoFrame(const std::vector<uint8_t> &encoded_data) {
  // Placeholder decode
  return cv::Mat();
}
