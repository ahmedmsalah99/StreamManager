...
struct FrameMetadata {
    ...
    FrameMetadata() :  timestamp_ns_system(0), fps(0.0), width(0), height(0), channels(0), cv_type(0), data_size(0) {}
    FrameMetadata(uint64_t ts_ns_system, double current_fps, uint32_t w, uint32_t h, uint32_t ch, uint32_t type, std::shared_ptr<std::vector<uint8_t>> data, size_t size)
        : timestamp_ns_system(ts_ns_system), fps(current_fps), width(w), height(h), channels(ch), cv_type(type), owned_data(std::move(data)), data_size(size) {}
};
class FrameBuffer {
public:
    ...
    // New: search by system-clock nanoseconds to avoid std::chrono in higher layers
    std::shared_ptr<FrameMetadata> getClosestFrameMetadataBySystemNs(uint64_t target_time_ns) const;
...
private:
    ...
    bool storeFrameInOwnedBuffer(const cv::Mat& frame, double current_fps, uint64_t ts_ns_system);
