...
int main() {
  ...
  std::cout << "  Validation errors: " << config_manager.getValidationErrors()
            << std::endl;

  // Test 3: Video source type conversion
  assert(config_manager.stringToSourceType("usb_camera") ==
         VideoSourceConfig::SourceType::USB_CAMERA);
  assert(config_manager.stringToSourceType("gazebo_ros2") ==
         VideoSourceConfig::SourceType::GAZEBO_ROS2);
  assert(config_manager.stringToSourceType("mavlink") ==
         VideoSourceConfig::SourceType::MAVLINK);
